...
class LargeTagDecoderTestCase(BaseTestCase):
    def testLargeTag(self):
        assert decode_one(ints2octs((127, 141, 245, 182, 253, 47, 3, 2, 1, 1))) == (1, null)

    def testLongTag(self):
        assert decode_one(ints2octs((0x1f, 2, 1, 0)))[0].tagSet == univ.Integer.tagSet

    def testTagsEquivalence(self):
        integer = univ.Integer(2).subtype(implicitTag=tag.Tag(tag.tagClassContext, 0, 0))
        assert decode_one(ints2octs((0x9f, 0x80, 0x00, 0x02, 0x01, 0x02)), asn1Spec=integer) == decode_one(
            ints2octs((0x9f, 0x00, 0x02, 0x01, 0x02)), asn1Spec=integer)


class DecoderCacheTestCase(BaseTestCase):
    def testCache(self):
        assert decode_one(ints2octs((0x1f, 2, 1, 0))) == decode_one(ints2octs((0x1f, 2, 1, 0)))


class IntegerDecoderTestCase(BaseTestCase):
    def testPosInt(self):
        assert decode_one(ints2octs((2, 1, 12))) == (12, null)

    def testNegInt(self):
        assert decode_one(ints2octs((2, 1, 244))) == (-12, null)

    def testZero(self):
        assert decode_one(ints2octs((2, 0))) == (0, null)

    def testZeroLong(self):
        assert decode_one(ints2octs((2, 1, 0))) == (0, null)

    def testMinusOne(self):
        assert decode_one(ints2octs((2, 1, 255))) == (-1, null)
...
class BooleanDecoderTestCase(BaseTestCase):
    def testTrue(self):
        assert decode_one(ints2octs((1, 1, 1))) == (1, null)

    def testTrueNeg(self):
        assert decode_one(ints2octs((1, 1, 255))) == (1, null)

    def testExtraTrue(self):
        assert decode_one(ints2octs((1, 1, 1, 0, 120, 50, 50))) == (1, ints2octs((0, 120, 50, 50)))

    def testFalse(self):
        assert decode_one(ints2octs((1, 1, 0))) == (0, null)

    def testTagFormat(self):
        try:
            decode_one(ints2octs((33, 1, 1)))
        except PyAsn1Error:
            pass
        else:
            assert 0, 'wrong tagFormat worked out'
...
class BitStringDecoderTestCase(BaseTestCase):
    ...
    def testTypeChecking(self):
        try:
            decode_one(ints2octs((35, 4, 2, 2, 42, 42)))
        except PyAsn1Error:
            pass
        else:
...
class NullDecoderTestCase(BaseTestCase):
    def testNull(self):
        assert decode_one(ints2octs((5, 0))) == (null, null)

    def testTagFormat(self):
        try:
            decode_one(ints2octs((37, 0)))
        except PyAsn1Error:
            pass
        else:
            assert 0, 'wrong tagFormat worked out'
...
if sys.version_info[0:2] > (2, 5):
    class UniversalStringDecoderTestCase(BaseTestCase):
        def testDecoder(self):
            assert decode_one(ints2octs((28, 12, 0, 0, 0, 97, 0, 0, 0, 98, 0, 0, 0, 99))) == (char.UniversalString(sys.version_info[0] == 3 and 'abc' or unicode('abc')), null)


class BMPStringDecoderTestCase(BaseTestCase):
    def testDecoder(self):
        assert decode_one(ints2octs((30, 6, 0, 97, 0, 98, 0, 99))) == (char.BMPString(sys.version_info[0] == 3 and 'abc' or unicode('abc')), null)


class UTF8StringDecoderTestCase(BaseTestCase):
    def testDecoder(self):
        assert decode_one(ints2octs((12, 3, 97, 98, 99))) == (char.UTF8String(sys.version_info[0] == 3 and 'abc' or unicode('abc')), null)
...
class SequenceOfWithExpTaggedOctetStringDecoder(BaseTestCase):
    ...
    def testDefModeSchema(self):
        s, r = decode_one(ints2octs((48, 5, 163, 3, 4, 1, 113)), asn1Spec=self.s)
        assert not r
        assert s == self.s
        assert s.tagSet == self.s.tagSet

    def testIndefModeSchema(self):
        s, r = decode_one(ints2octs((48, 128, 163, 128, 4, 1, 113, 0, 0, 0, 0)), asn1Spec=self.s)
        assert not r
        assert s == self.s
        assert s.tagSet == self.s.tagSet

    def testDefModeNoComponent(self):
        s, r = decode_one(ints2octs((48, 5, 163, 3, 4, 1, 113)), asn1Spec=self.s2)
        assert not r
        assert s == self.s
        assert s.tagSet == self.s.tagSet

    def testIndefModeNoComponent(self):
        s, r = decode_one(ints2octs((48, 128, 163, 128, 4, 1, 113, 0, 0, 0, 0)), asn1Spec=self.s2)
        assert not r
        assert s == self.s
        assert s.tagSet == self.s.tagSet

    def testDefModeSchemaless(self):
        s, r = decode_one(ints2octs((48, 5, 163, 3, 4, 1, 113)))
        assert not r
        assert s == self.s
        assert s.tagSet == self.s.tagSet

    def testIndefModeSchemaless(self):
        s, r = decode_one(ints2octs((48, 128, 163, 128, 4, 1, 113, 0, 0, 0, 0)))
        assert not r
        assert s == self.s
        assert s.tagSet == self.s.tagSet
...
class ChoiceDecoderTestCase(BaseTestCase):
    ...
    def testWithoutSpec(self):
        self.s.setComponentByPosition(0, univ.Null(null))
        assert decode_one(ints2octs((5, 0))) == (self.s, null)
        assert decode_one(ints2octs((5, 0))) == (univ.Null(null), null)

    def testUndefLength(self):
        self.s.setComponentByPosition(2, univ.OctetString('abcdefgh'))
        assert decode_one(ints2octs((36, 128, 4, 3, 97, 98, 99, 4, 3, 100, 101, 102, 4, 2, 103, 104, 0, 0)),
    ...
    def testExplicitTag(self):
        ...
        assert decode_one(ints2octs((164, 2, 5, 0)), asn1Spec=s) == (s, null)

    def testExplicitTagUndefLength(self):
        s = self.s.subtype(explicitTag=tag.Tag(tag.tagClassContext,
                                               tag.tagFormatConstructed, 4))
        s.setComponentByPosition(0, univ.Null(null))
        assert decode_one(ints2octs((164, 128, 5, 0, 0, 0)), asn1Spec=s) == (s, null)
...
class AnyDecoderTestCase(BaseTestCase):
    ...
    def testTaggedEx(self):
        ...
        assert decode_one(ints2octs((164, 5, 4, 3, 102, 111, 120)), asn1Spec=s) == (s, null)

    def testTaggedIm(self):
        s = univ.Any('\004\003fox').subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 4))
        assert decode_one(ints2octs((132, 5, 4, 3, 102, 111, 120)), asn1Spec=s) == (s, null)
    ...
    def testTaggedExIndefMode(self):
        s = univ.Any('\004\003fox').subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 4))
        assert decode_one(ints2octs((164, 128, 4, 3, 102, 111, 120, 0, 0)), asn1Spec=s) == (s, null)
...
class EndOfOctetsTestCase(BaseTestCase):
    ...
    def testExpectedEoo(self):
        result, remainder = decode_one(ints2octs((0, 0)), allowEoo=True)
    ...
    def testDefiniteNoEoo(self):
        try:
            decode_one(ints2octs((0x23, 0x02, 0x00, 0x00)))
        except PyAsn1Error:
            pass
        else:
    ...
    def testNoConstructedEoo(self):
        try:
            decode_one(ints2octs((0x23, 0x80, 0x20, 0x00)))
        except PyAsn1Error:
            pass
        else:
    ...
    def testNoEooData(self):
        try:
            decode_one(ints2octs((0x23, 0x80, 0x00, 0x01, 0x00)))
        except PyAsn1Error:
            pass
        else:
